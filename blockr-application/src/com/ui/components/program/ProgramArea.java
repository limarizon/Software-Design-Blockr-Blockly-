...
public class ProgramArea extends com.ui.Container {
    ...
    */
    ...
    public void buildProgramBlockComponentFromRoot(Block root, WindowPosition rootPosition){
        ...
        if(root instanceof ControlFlowBlock){
            buildProgramBlockComponentFromRoot(((ControlFlowBlock) root).getBody(),rootPosition.plus(new WindowPosition(BlockSizes.CONTROL_FLOW_INNER_START, BlockSizes.CONDITION_BLOCK_HEIGHT)));
            buildProgramBlockComponentFromRoot(((ControlFlowBlock) root).getCondition(),rootPosition.plus(new WindowPosition(BlockSizes.BLOCK_WIDTH,0)));
            ...
            if(root instanceof WallInFrontBlock){
                ...
                newb.setViewContext(this.getViewContext());
                programBlockComponents.add(newb);
                buildProgramBlockComponentFromRoot(((NotBlock) root).getCondition(),rootPosition.plus(new WindowPosition(BlockSizes.CONDITION_BLOCK_WIDTH,0)));
            }
        }
    }
    */
